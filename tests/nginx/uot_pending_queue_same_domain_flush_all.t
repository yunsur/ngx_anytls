...
use AnyTLS::Test qw/
    ...
    udp_echo_socket udp_log_daemon dns_socket dns_daemon
/;
use Test::Nginx;
...
my $udp_log = $t->testdir() . '/udp.log';

$t->run_daemon(sub {
    dns_daemon($dns_server, { 'batch.test' => 'DELAY:0.3:127.0.0.1' });
});
$t->run_daemon(sub { udp_log_daemon($udp_server, $udp_log) });
$t->run();
usleep(100_000);
...
$client->write(frame(CMD_PUSH, $stream_id,
    uot_domain_packet('batch.test', $udp_port, 'batch one')));
$client->write(frame(CMD_PUSH, $stream_id,
    uot_domain_packet('batch.test', $udp_port, 'batch two')));

my (undef, undef, $first) = parse_uot_ipv4_packet(read_anytls_push($client, $stream_id));

is($first, 'batch one', 'first pending UoT domain packet flushes after resolve');

usleep(100_000);

open my $log, '<', $udp_log or die "failed to read UDP log: $!";
my @received = <$log>;
close $log;
chomp @received;

ok(grep($_ eq 'batch one', @received),
    'UDP upstream receives first same-domain pending packet');
ok(grep($_ eq 'batch two', @received),
    'UDP upstream receives second same-domain pending packet');
is(scalar grep($_ =~ /^batch /, @received), 2,
    'UDP upstream receives both same-domain pending packets exactly once');
