...
$client->write(frame(CMD_PUSH, $stream_id,
    uot_domain_packet('fast.test', $udp_port, 'warm cache')));
my (undef, undef, $warm) = parse_uot_ipv4_packet(read_anytls_push($client, $stream_id));
is($warm, 'warm cache', 'first packet warms UoT domain cache');

$client->write(frame(CMD_PUSH, $stream_id,
    uot_domain_packet('slow.test', $udp_port, 'stuck pending')));
$client->write(frame(CMD_PUSH, $stream_id,
    uot_domain_packet('fast.test', $udp_port, 'cached bypass')));

my (undef, undef, $bypass) = parse_uot_ipv4_packet(read_anytls_push($client, $stream_id));
is($bypass, 'cached bypass', 'cached UoT domain packet bypasses pending queue head');
