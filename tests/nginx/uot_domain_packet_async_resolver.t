...
$t->run_daemon(sub { dns_daemon($dns_server, { 'async.test' => '127.0.0.1' }) });
...
my $stream_id = 11;
my $payload = 'uot async resolver echo';
...
$client->write(frame(CMD_PUSH, $stream_id,
    uot_domain_packet('async.test', $udp_port, $payload)));

my ($host, $port, $body) = parse_uot_ipv4_packet(read_anytls_push($client, $stream_id));

is($host, '127.0.0.1', 'async resolver returns IPv4 localhost');
is($port, $udp_port, 'async resolver UoT response source port matches UDP upstream');
is($body, $payload, 'UoT domain packet resolves through nginx async resolver');
