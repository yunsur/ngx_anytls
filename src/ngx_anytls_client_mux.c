    ...
    ngx_anytls_stream_t *st, ngx_anytls_out_frame_t *f)
{
    ...
    ngx_anytls_client_mux_post_write(ac);
...
ngx_int_t
ngx_anytls_client_mux_queue_frame(ngx_anytls_connection_t *ac, ngx_anytls_stream_t *st,
...
ngx_int_t
ngx_anytls_client_mux_queue_ref_frame(ngx_anytls_connection_t *ac, ngx_anytls_stream_t *st,
    ngx_uint_t cmd, uint32_t stream_id, u_char *data, size_t len)
{
    ngx_anytls_out_frame_t *f;
...
ngx_int_t
ngx_anytls_client_mux_send_synack(ngx_anytls_stream_t *st, u_char *data, size_t len)
{
    ...
    return ngx_anytls_client_mux_queue_ref_frame(st->ac, st, NGX_ANYTLS_CMD_SYNACK,
                                      st->id, data, len);
}
    ...
    size_t payload_len)
{
    ...
    return ngx_anytls_client_mux_has_room(ac, payload_len);
}
...
{
    return ngx_anytls_client_mux_queue_ref_frame(ac, st, NGX_ANYTLS_CMD_ALERT,
                                      st ? st->id : 0, data, len);
}
